...
class ListNode{
	...
	public ListNode link; // 다른 노드를 참조할 링크
	public ListNode() {
		this.data=null;
		this.link=null;
	}
...
public class UserLinkedList {
	...
	public void insertNode(ListNode preNode, Object data)
	{
		...
		// preNode.link 는 preNode의 다음 노드이므로
	...
	public void insertNode(Object data) {
		...
		if(head==null) {
			...
			//while문이 모두 실행되면 tempNode는 가장 마지막에 있는 노드를 참조하게 됨.
			while(tempNode.link!=null)
			{
				tempNode=tempNode.link; // 다음 노드를 참조
			}
			// tempNode(가장 마지막 노드)의 link가 다음 노드를 참조하도록 함.
	...
	public void deleteNode() {
		...
		//head 노드가 null 인 경우 모든 노드가 삭제되었으므로 return
		if(head==null) {
			return;
		}
		
		//head 노드의 link 가 null 인 경우
		//노드가 1개 남았을 경우
		if(head.link == null) {
	...
	public ListNode searchNode(Object data) {
		...
		//tempNode가 null 이 아닐 때까지 반복하여 탐색
		while(tempNode !=null)
		{
	...
	public void reverseList() {
		...
		while(nextNode!=null) {
			...
			currentNode.link= preNode; // currentNode의 link에 preNode를 할당시켜 역순으로 설정
		}
	...
	public void printList() {
		...
		//tempNode가 null이 아닐 때까지 반복해서 출력
		while(tempNode!=null) {
			System.out.print(tempNode.getData() + " ");
			tempNode = tempNode.link; // temp 노드에 다음 노드(tempNode.link)를 할당.
		}
		System.out.println();
	}
	public int getSize() {
	...
	public static void main(String[] args) {
		UserLinkedList linkedList = new UserLinkedList();
		String str= "wed";
